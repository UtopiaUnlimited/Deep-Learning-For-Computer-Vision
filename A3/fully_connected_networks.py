...
"""
from numpy._typing import _96Bit
import torch
...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = torch.mm(x.reshape(x.shape[0], -1), w) + b.reshape(-1, b.shape[0])
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        t, lr_cache = Linear_ReLU.forward(X, self.params["W1"], self.params["b1"])
        scores, fc_cache = Linear.forward(t, self.params["W2"], self.params["b2"])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dt, grads["W2"], grads["b2"] = Linear.backward(dout, fc_cache)
        grads["W2"] += 2 * self.reg * self.params["W2"]
        _, grads["W1"], grads["b1"] = Linear_ReLU.backward(dt, lr_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for hidden_dim in hidden_dims:
          self.params[f"W{i+1}"] = torch.randn(last_hidden_dim, hidden_dim, dtype = dtype, device = device) * weight_scale
          self.params[f"b{i+1}"] = torch.zeros(hidden_dim, dtype = dtype, device = device)
          last_hidden_dim = hidden_dim
          i += 1

        self.params[f"W{len(hidden_dims)+1}"] = torch.randn(hidden_dims[-1], num_classes, dtype = dtype, device = device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          out, lr_cache = Linear_ReLU.forward(last_out, self.params[f"W{i+1}"], self.params[f"b{i+1}"])
          if self.use_dropout:
            out, dropout_cache = Dropout.forward(out, self.dropout_param)
            dropout_caches.append(dropout_cache)
          caches.append(lr_cache)
          last_out = out
        out, fc_cache = Linear.forward(last_out, self.params[f"W{self.num_layers}"], self.params[f"b{self.num_layers}"])
        ...
        # Replace "pass" statement with your code
        loss, dout = softmax_loss(scores, y)
        dout, grads[f"W{self.num_layers}"], grads[f"b{self.num_layers}"] = Linear.backward(dout, caches[self.num_layers - 1])
        ...
        for i in range(self.num_layers - 1)[::-1]:
            ...
            dout, grads[f"W{i+1}"], grads[f"b{i+1}"] = Linear_ReLU.backward(dout, caches[i])
...
def adam(w, dw, config=None):
    ...
    m_no = config["m"] / (1 - config["beta1"] ** config["t"])
    v_no = config["v"] / (1 - config["beta2"] ** config["t"])
    next_w = w - config["learning_rate"] * m_no / (v_no.sqrt() + config["epsilon"])
    #########################################################################
    #                              END OF YOUR CODE                         #
    #########################################################################
